<script lang="ts">
	...
	let summary = $state(data.page.summary ?? '');
	let content = $state(data.page.content ?? '');
	let metaTitle = $state(data.page.meta_title ?? '');
	let metaDescription = $state(data.page.meta_description ?? '');
	let isPublished = $state(data.page.is_published);
	let sitemapPriority = $state(data.page.sitemap_priority ?? 0.7);
	let sitemapChangefreq = $state(data.page.sitemap_changefreq ?? 'weekly');
	...
	const previewTitle = $derived(
		(metaTitle.trim() || title.trim() || 'Page Title') + ' | AllCare Refrigeration'
	);
...
<div class="editor-container">
	...
	<header class="editor-header">
		<div>
			<h2 class="title">Edit Page: {data.page.title}</h2>
			<div class="header-meta">
				<span class="url-hint">
					Live URL:
					<a href="/{slug}" target="_blank" rel="noopener noreferrer">
						https://www.allcarerefrigeration.com/{slug} ↗
					</a>
				</span>
			</div>
		</div>
		<div class="header-actions">
			<a href="/{slug}" target="_blank" rel="noopener noreferrer" class="btn-outline">
				View Page ↗
			</a>
		</div>
	</header>
	...
	{#if showSuccessBanner}
		<div class="success-banner">
			✓ Page changes saved successfully! Sitemap and public routes are updated.
		</div>
	{/if}
			...
			<div class="sidebar-column">
				...
				<div class="card">
					...
					<div class="form-group">
						...
						<p class="field-hint">
							{isPublished
								? 'Page is live publicly and included in the XML sitemap.'
								: 'Page is saved as Draft (only accessible to authenticated admins).'}
						</p>
					</div>
					...
					<div class="form-actions">
						...
						<a href="/admin/pages" class="btn-secondary btn-block">Back to Pages</a>
					</div>
				</div>
				...
				<div class="card">
					...
					<div class="form-group">
						...
						<span class="field-hint">Informs search engines how frequently this content updates.</span>
					</div>
				</div>

				<!-- Danger Zone / Delete Card -->
				<div class="card card-danger">
					<h3 class="card-title text-danger">Delete Page</h3>
					<p class="field-hint">
